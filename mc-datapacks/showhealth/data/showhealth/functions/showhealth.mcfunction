schedule function showdealth 1s
...
execute as @a at @s if entity @e[distance=..10,sort=nearest,limit=1,nbt={DeathTime:0s},type=!armor_stand,type=!minecraft:player] run title @s actionbar ["",{"selector":"@e[distance=..10,sort=nearest,limit=1,nbt={DeathTime:0s},type=!armor_stand,type=!minecraft:player]}"},{"text":"血量:","bold":true,"color":"gold"},{"score":{"name":"@s","objective":"show_health"}}]
