...
execute as @s[scores={fishing.fishY=..-10},tag=fishing,tag=owned] at @s run execute as @a if score @s uuid3 = @e[type=minecraft:fishing_bobber,tag=fishing,tag=owned,sort=nearest,limit=1] uuid3 run function fishing:player with storage fishing:player
